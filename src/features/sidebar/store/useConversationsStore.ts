...
const sortByPinnedAt = (conversations: Conversation[]): Conversation[] => {
  // Bolt Optimization: Sort in-place to avoid redundant array cloning.
  // The input arrays are already freshly created clones in all call sites.
...
const mergeConversations = (
  ...
  // Bolt Optimization: Pass map.values() directly to avoid an unnecessary O(N) array allocation
...
>((set, get) => ({
  ...
  updateConversationTitle: async (id, title) => {
    ...
    // Bolt Optimization: Use sequential find calls to avoid concatenating arrays.
    // This removes a full array allocation that gets increasingly expensive with more conversations.
