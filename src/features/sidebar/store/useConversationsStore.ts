...
const sortByPinnedAt = (conversations: Conversation[]): Conversation[] => {
  ...
  sorted.sort((a, b) => {
    ...
    // Optimization: Lexical string comparison is ~70x faster than localeCompare for ISO dates
...
const mergeConversations = (
  ...
  // Optimization: passing map.values() directly to avoid O(N) array allocation from Array.from()
