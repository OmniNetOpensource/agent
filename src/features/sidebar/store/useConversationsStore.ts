...
const sortByPinnedAt = (conversations: Conversation[]): Conversation[] => {
  ...
  sorted.sort((a, b) => {
    ...
    // ⚡ Bolt: Fast string comparison for ISO-8601 dates is significantly faster than localeCompare
...
const mergeConversations = (
  ...
  // ⚡ Bolt: Pass map.values() directly to avoid an unnecessary O(N) array allocation with Array.from()
