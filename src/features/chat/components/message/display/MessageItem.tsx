...
const areBranchInfosEqual = (
  ...
  if (prev.siblingIds.length !== next.siblingIds.length) return false;
  for (let i = 0; i < prev.siblingIds.length; i++) {
    if (prev.siblingIds[i] !== next.siblingIds[i]) return false;
  }
  return true;
};
