...
export function MessageList() {
  ...
  }, []); // Only bind native listener once to prevent thrashing

  // Handle position checks triggered by message updates (e.g. streaming content)
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
    ...
    const atBottom = distanceToBottom <= 32;
    setIsAtBottom((prev) => (prev !== atBottom ? atBottom : prev));
  }, [messages]); // Use messages reference, not messages.length for streaming support
