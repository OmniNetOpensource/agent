...
export function MessageList() {
  ...
  const lastMessageRole = useMessageTreeStore((state) => {
    const lastId = state.currentPath[state.currentPath.length - 1];
    return lastId ? state.messages[lastId - 1]?.role : null;
  });
    ...
    <div className="relative h-full w-full">
      ...
      >
        ...
        >
          ...
          {pending && lastMessageRole === "user" && <PendingIndicator />}
        </div>
      </div>
      ...
      {currentPath.length > 0 && !isAtBottom && (
