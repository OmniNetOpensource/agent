...
export function MessageList() {
  ...
  }, []); // Only bind native scroll listener once

  // Re-evaluate position when messages change (e.g. streaming, new messages)
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
    ...
    const atBottom = distanceToBottom <= 32;
    setIsAtBottom((prev) => (prev !== atBottom ? atBottom : prev));
  }, [messages]); // Dependency on stable messages array reference
