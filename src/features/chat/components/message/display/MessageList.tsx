...
export function MessageList() {
  ...
  // ⚡ Bolt: Memoize messages to prevent downstream components from re-rendering
  // on unrelated state changes (like 'pending' below)
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // ⚡ Bolt: Use functional state update to prevent reading from closure
  ...
  }, []); // ⚡ Bolt: Empty dependency array binds listener exactly once

  // ⚡ Bolt: Separate effect for position checks when new messages arrive,
  // avoiding listener re-binding during streaming
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
