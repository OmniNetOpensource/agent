...
export function MessageList() {
  ...
  // Memoize to prevent unnecessary new array references on every render,
  // which causes cascading re-renders in child components.
  ...
  // Separate effect to handle scroll event binding exactly once
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Use functional state update to prevent listener thrashing
    ...
    // Bind passive listener for smooth scrolling
    container.addEventListener("scroll", handleScroll, { passive: true });

    return () => {
      container.removeEventListener("scroll", handleScroll);
    };
  }, []); // Empty dependency array prevents listener thrashing during streaming

  // Separate effect to check position when new messages arrive
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) return;
