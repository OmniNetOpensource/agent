...
export function MessageList() {
  ...
  }, []); // Empty dependency array to bind listener exactly once

  // Extract the position check logic triggered by new messages into a separate useEffect
  useEffect(() => {
    const container = scrollRef.current;
    if (!container) {
      return;
    ...
    setIsAtBottom((prev) => (prev !== atBottom ? atBottom : prev));
  }, [messages]); // Keep messages as dependency so that if the list height grows (e.g., during streaming) we update isAtBottom properly
